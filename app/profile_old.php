...
class Profile extends Model {
    ...
    public static function profiles($where = null, $having = null, $orderBy = null, $limit = null, $offset = null, $count = null) {
        $results = DB::select("select `u`.*, CONCAT(`u`.`first_name`,' ',`u`.`last_name`) AS `name`,
            ...
            .(!empty($count)?"":(empty($orderBy)?"":" order by ".$orderBy))
            .(!empty($count)?"":(empty($limit)?"":" limit ".(empty($offset)?"":$offset.", ").$limit))
        );
        if (!empty($count))
